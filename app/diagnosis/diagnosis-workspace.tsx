...
export default function DiagnosisWorkspace({ initial }: { initial?: { vin: string; vehicle: string; symptoms: string; workOrderId: string; workOrderNumber: string } }) {
  const [vin, setVin] = useState(initial?.vin ?? "");
  const [vehicle, setVehicle] = useState(initial?.vehicle ?? "2010 Honda Accord 2.4L");
